...
namespace TAOBOT8
{
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            string token = "6246993187:AAE28LGgr-J5Rp-Hi3C0DcTGfhnHqfnd--s";
            ...
            async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
            {
                ...
                {
                    reply = "FeedBack Giáo viên:🥲 Học lập trình từ thầy Đỗ Duy Cốp";
                }
                else if (new[] { "mh" }.Any(keyword => messLow.Contains(keyword)))
                {
                    string tenMH = messageText.Substring(messageText.IndexOf(' ') + 1);
                    reply = ketnoivoianh.Tim_mh("%" + tenMH.Replace(' ', '%') + "%");
                }
                ...
                else if (new[] { "duy", "anh duy" }.Any(keyword => messLow.Contains(keyword)))
                {
                    reply = "Ý bạn là anh Phương Minh Duy đẹp trai đúng không? " +
                        "Tôi có thể cho bạn biết, anh ấy là 1 chàng trai tuyệt vơi"
                        ;
                }
                else if (messLow.StartsWith("thế định"))
                {
                    reply = " ý bạn là con chó thế định, ở bắc ninh phải không ";
                }

                else if (messLow.StartsWith("tôi muốn qua môn"))
                {
                    reply = " khó lắm bạn ơi, tôi không giúp được cho bạn ";
                }
                else if (new[] { "tiến", "thằng tiến" }.Any(keyword => messLow.Contains(keyword)))
                {
                    reply = " nó là 1 con chó, tôi bắt nó câm ngay cho bạn";
                }
